...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password) {
        $stmt = $conn->prepare("UPDATE users SET firstname = ?, lastname = ?, username = ?, password = ?, phone = ?, address = ?, profile_picture = ? WHERE id = ?");
        $stmt->execute([$firstName, $lastName, $username, password_hash($password, PASSWORD_DEFAULT), $phone, $address, $profilePicture, $userId]);
    } else {
        $stmt = $conn->prepare("UPDATE users SET firstname = ?, lastname = ?, username = ?, phone = ?, address = ?, profile_picture = ? WHERE id = ?");
        $stmt->execute([$firstName, $lastName, $username, $phone, $address, $profilePicture, $userId]);
    }
        ...
        ?>
            ...
            <form action="profile_setting.php" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <small class="form-text text-muted">We regret to inform you that you do not have the necessary permissions to change the email address.</small>
                </div>
                <div class="mb-3">
